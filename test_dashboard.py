...
class FakeRuntime:
    def __init__(self):
        ...
        self._states = {}
        self.nodes = [
            {"nodeID": "mac-1", "lastSeenAt": 990, "platform": "macOS",
             "hardwareIdentifier": "Mac14,7", "capabilities": {"gpuName": "Apple M2", "workloads": ["openstar.lomb-scargle.v1"]}},
            {"nodeID": "phone-1", "lastSeenAt": 800, "platform": "iOS",
             "hardwareIdentifier": "iPhone15,2", "capabilities": {"batteryLevel": 0.72}},
    ...
    def contribution_summary(self):
        devices = [{"nodeID": "mac-1", "acceptedWorkUnits": 4,
                    "workerComputeSeconds": 12.0, "metalSeconds": 10.0,
                    "sampleFrequencyEvaluationsPerMetalSecond": 25.0}]
        scope = {"totalAcceptedWorkUnits": 4, "totalWorkerComputeSeconds": 12.0,
...
class DashboardProjectionTests(unittest.TestCase):
    def test_classification_aggregation_and_optional_telemetry(self):
        ...
        self.assertEqual({"macOS", "iOS", None}, {w["platform"] for w in result["workers"]})
    ...
    def test_active_assignment_wins_over_idle(self):
        runtime = FakeRuntime()
        state = SimpleNamespace(lock=threading.RLock(), project_id="project", workload_id="generic",
            assigned={"unit": {"nodeID": "mac-1", "assignedAt": 995, "leaseExpiresAt": 1115}},
            work_units={"unit": {"projectID": "project", "workloadID": "generic", "datasetID": "data"}},
            completed={}, execution_failure_history={})
    ...
    def test_activity_is_read_only(self):
        runtime = FakeRuntime()
        state = SimpleNamespace(lock=threading.RLock(), project_id="p", project_name="Project",
            workload_id="generic", work_units={"u": {}}, pending=["u"], assigned={},
            completed={}, failed={})
        runtime._states["p"] = state
        before = list(state.pending)
        self.assertEqual(1, activity_snapshot(runtime)["projects"][0]["projectPendingWorkUnits"])
...
class DashboardAPITests(unittest.TestCase):
    ...
    def tearDownClass(cls):
        cls.server.shutdown(); cls.server.server_close(); cls.patcher.stop()
    ...
    def test_worker_detail_and_missing_worker(self):
        self.assertEqual("mac-1", json.loads(self.get("/api/dashboard/workers/mac-1")[2])["id"])
