...
def _assignment_snapshot(runtime: Any) -> tuple[dict[str, dict[str, Any]], Counter, list]:
    ...
    for state in states:
        with state.lock:
            for work_id, lease in state.assigned.items():
                ...
                assignments[str(lease.get("nodeID", "")).lower()] = {
                    "workUnitID": work_id,
            ...
            for result in state.completed.values():
                if result.get("nodeID") is not None:
                    recent.append({
                        ...
                        "workUnitID": result.get("id") or result.get("workUnitID"),
                        "projectID": state.project_id,
                        "workloadID": state.workload_id,
                        "durationSeconds": result.get("duration"),
                    })
            for items in state.execution_failure_history.values():
                for item in items:
                    failures[str(item.get("nodeID", "")).lower()] += 1
    return assignments, failures, recent[-100:]
...
def dashboard_snapshot(runtime: Any, now: float | None = None) -> dict[str, Any]:
    ...
    assignments, failures, recent = _assignment_snapshot(runtime)
    workers = []
    for source in nodes:
        ...
        assignment = assignments.get(node_id.lower())
        if age is not None and age <= CONNECTED_SECONDS:
            connection = "connected"
            compute = "active" if assignment else "idle"
        ...
        workers.append({
            ...
            "workerVersion": _value(source, "appVersion", "workerVersion", "version"),
            ...
            "measuredThroughput": totals.get("sampleFrequencyEvaluationsPerMetalSecond"),
    ...
    summary = {
        "knownWorkers": len(workers), "connectedWorkers": connected,
        "activeWorkers": counts["active"], "idleWorkers": counts["idle"],
        "offlineWorkers": counts["offline"],
        "recentlyDisconnectedWorkers": sum(w["connectionState"] == "recently_disconnected" for w in workers),
        "completedWorkUnits": contributions["allTime"]["totalAcceptedWorkUnits"],
        "runningWorkUnits": len(assignments),
        ...
        "measuredThroughput": current["aggregateSampleFrequencyEvaluationsPerMetalSecond"],
        "throughputUnit": "sample-frequency evaluations / Metal second",
        "failureCount": sum(failures.values()),
        "health": "degraded" if failures and connected == 0 else ("healthy" if connected else "quiet"),
        "updatedAt": now,
    }
    return {"summary": summary, "workers": workers, "recentCompleted": recent}
...
def history_snapshot(runtime: Any) -> dict[str, Any]:
    ...
    for node in summary.get("nodes", []):
        by_model[node.get("hardwareIdentifier") or "Unknown"] += node["acceptedWorkUnits"]
    return {"available": False, "reason": "Time-series telemetry begins with a future ledger schema.",
            "contributionByWorker": summary.get("nodes", []),
            "completedByDeviceModel": dict(by_model), "series": []}
